...
public sealed class Win10MsPinyinSelfStudy : IWordLibraryExport, IWordLibraryImport
{
    ...
    private void ExportTo1File(string tempPath, WordLibraryList wlList)
    {
        ...
        for (var i = 0; i < 9192; i++) bw.Write((byte)0);
        //0x2400词条开始
        for (var i = 0; i < wlList.Count; i++)
        {
            var wl = wlList[i];
            try
            {
                ...
                bw.Write(BitConverter.GetBytes((short)(i + 0x6D1B))); //Unknown，怀疑是词频
        ...
        while (fs.Position < k * 1024) bw.Write((byte)0);
    ...
    private byte[] HexStringToByteArray(string s)
    {
        ...
        for (var i = 0; i < s.Length; i += 2) buffer[i / 2] = Convert.ToByte(s.Substring(i, 2), 16);
    ...
    private Dictionary<string, short> PinyinMap
    {
        get
        {
            if (pinyinMap == null)
            {
                ...
                for (short i = 0; i < pinyinIndex.Length; i++) pinyinMap[pinyinIndex[i]] = i;
            }
    ...
    private static readonly string[] pinyinIndex = new[]
    {
        "a",
        "ai",
        "an",
