...
public sealed class LingoesLd2 : BaseImport, IWordLibraryImport
{
    ...
    private readonly Regex regex = new("[\u4E00-\u9FA5]+");
    ...
    private IList<string> Parse(string ld2File)
    {
        using (var fs = new FileStream(ld2File, FileMode.Open, FileAccess.Read))
        {
            ...
            fs.Position = 0x5c;
            var offsetData = BinFileHelper.ReadInt32(fs) + 0x60;
            if (fs.Length > offsetData)
            {
                Debug.WriteLine("简介地址：0x" + offsetData.ToString("x"));
                fs.Position = offsetData;
                var type = BinFileHelper.ReadInt32(fs);
                Debug.WriteLine("简介类型：0x" + type.ToString("x"));
                fs.Position = offsetData + 4;
                var offsetWithInfo = BinFileHelper.ReadInt32(fs) + offsetData + 12;
                if (type == 3)
                    // without additional information
                    return ReadDictionary(fs, offsetData);
                if (fs.Length > offsetWithInfo - 0x1C) return ReadDictionary(fs, offsetWithInfo);
    ...
    private IList<string> ReadDictionary(FileStream fs, int offsetWithIndex)
    {
        ...
        var limit = BinFileHelper.ReadInt32(fs) + offsetWithIndex + 8; //文件结束地址
        var offsetIndex = offsetWithIndex + 0x1C; //索引开始的地址
        var offsetCompressedDataHeader = BinFileHelper.ReadInt32(fs) + offsetIndex; //索引结束，数据头地址
        var inflatedWordsIndexLength = BinFileHelper.ReadInt32(fs);
        var inflatedWordsLength = BinFileHelper.ReadInt32(fs);
        var inflatedXmlLength = BinFileHelper.ReadInt32(fs);
        var definitions = (offsetCompressedDataHeader - offsetIndex) / 4;
    ...
    //}
    private readonly Regex hregex = new(@"<(.[^>]*)>");
    ...
    )
    {
        ...
        var xml = xmlStringDecoder.GetString(
            inflatedBytes,
