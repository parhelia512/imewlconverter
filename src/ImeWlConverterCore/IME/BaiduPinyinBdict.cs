...
public sealed class BaiduPinyinBdict : BaseImport, IWordLibraryImport
{
    ...
    private readonly List<string> Yunmu = new()
    {
        "uang",
        "iang",
        "iong",
    ...
    public WordLibraryList Import(string path)
    {
        ...
        do
        {
            ...
            try
            {
                ...
                if (wl == null) break;
                if (wl.Word != "" && wl.PinYin.Length > 0) wordLibraryList.Add(wl);
            }
            catch (Exception ex)
            {
    ...
    private WordLibrary ImportWord(FileStream fs)
    {
        ...
        //}
        if (show > 0) Debug.WriteLine(show + "  " + wordLibrary.Word + "----" + wordLibrary.PinYinString);
        return wordLibrary;
    }
