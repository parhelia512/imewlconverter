 ...
 * Licensed under the GNU General Public License v3.0.
 //
 * This work contains codes translated from the original work rose by nopdan (https://github.com/nopdan/rose)
 * Copyright nopdan
 * Licensed under the GNU General Public License v3.0.
 */
...
public sealed class SougouPinyinBinFromPython : BaseImport, IWordLibraryImport
{
    ...
    public WordLibraryList Import(string path)
    {
        ...
        if (header == 0x55504753) return NewParser(fs);
    ...
    private List<uint> ReadAInts(FileStream fs, uint length)
    {
        ...
        for (var i = 0; i < length; i++) list.Add(BinFileHelper.ReadUInt32(fs));
    ...
    private readonly string[] PinyinData =
    {
        "a",
        "ai",
        "an",
