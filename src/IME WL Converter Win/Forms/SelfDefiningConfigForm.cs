...
public partial class SelfDefiningConfigForm : Form
{
    ...
    {
        if (ReBuildUserPattern()) DialogResult = DialogResult.OK;
    }
    ...
    {
        if (cbxPinyinSplitBefore.Checked && cbxPinyinSplitBehind.Checked) return BuildType.FullContain;
        if (cbxPinyinSplitBefore.Checked) return BuildType.LeftContain;
        if (cbxPinyinSplitBehind.Checked) return BuildType.RightContain;
